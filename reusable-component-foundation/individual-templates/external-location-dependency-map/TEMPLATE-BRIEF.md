# External-location dependency map

Parent capability: Diagram and dependency-map primitives
Template type: composite

## Build this exact template

Map external sources, Databricks environments and external targets into three dependency lanes.
...
| --- | --- | --- | ---: | --- |
| UI renderer/controller | `buildExternalLocationGraph(...)` | `app/src/app.js` | 14722-14775 | `code/app--buildExternalLocationGraph.reference.js` |
| UI renderer/controller | `renderExternalLocationDependencyMap(...)` | `app/src/app.js` | 14789-14872 | `code/app--renderExternalLocationDependencyMap.reference.js` |
| UI renderer/controller | `renderExternalLocationDiagram(...)` | `app/src/app.js` | 14636-14720 | `code/app--renderExternalLocationDiagram.reference.js` |

Start with those focused extracts. Use the full source snapshot only for helpers they call.

## Inputs (props/template controls)

- External connection rows
- BU/environment context
...
1. Map external sources, Databricks environments and external targets into three dependency lanes.
...
- **Primary interaction:** Exercise the main output: Node/edge graph. Show the state change immediately.
...
- [diagram-toolbar](../diagram-toolbar/TEMPLATE-BRIEF.md)
