...
Show counts and coverage for the current metadata-review context.
...
| --- | --- | --- | ---: | --- |
| UI renderer/controller | `renderMetadataReviewStats(...)` | `app/src/app.js` | 14127-14139 | `code/app--renderMetadataReviewStats.reference.js` |

Start with those focused extracts. Use the full source snapshot only for helpers they call.

## Inputs (props/template controls)

- Metadata review model
- Context label
...
1. Show counts and coverage for the current metadata-review context.
...
- **Primary interaction:** Exercise the main output: KPI card grid. Show the state change immediately.
