...
Keep chart layouts stable and explain which source data is required when no values exist.
...
| --- | --- | --- | ---: | --- |
| UI renderer/controller | `renderAdvancedDiscoveryEmptyChart(...)` | `app/src/app.js` | 13308-13317 | `code/app--renderAdvancedDiscoveryEmptyChart.reference.js` |
...
1. Keep chart layouts stable and explain which source data is required when no values exist.
...
- **Primary interaction:** Exercise the main output: Accessible empty-state panel. Show the state change immediately.
